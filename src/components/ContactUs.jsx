...
const G = {
  ...
  tealBorder:'rgba(43,135,141,0.25)',
  darkSection:`linear-gradient(to right, rgb(20,68,71), rgb(43,135,141))`,
};
    ...
    <div className="min-h-screen relative overflow-x-hidden">
      <div className="md:ml-[280px]">
        ...
        <div className="fixed inset-0 z-0">
          ...
          <div className="absolute bottom-0 left-0 w-[500px] h-[500px] rounded-full blur-3xl opacity-25"
        ...
        <div className="relative z-10 py-8 sm:py-12 px-4 sm:px-6 lg:px-8">
          ...
          >
            <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-2 rounded-full border shadow-sm mb-4"
              style={{ background: G.glass40, borderColor: G.border60, backdropFilter: 'blur(10px)' }}>
              <Sparkles size={16} style={{ color: C.teal }} />
              <span className="text-xs sm:text-sm font-medium" style={{ color: C.darkTeal }}>Get in Touch</span>
            </div>
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-4" style={{ color: C.darkTeal }}>
              Let's <span style={{ color: C.teal }}>Connect</span>
            </h1>
            <p className="text-base sm:text-lg px-4 max-w-2xl mx-auto" style={{ color: 'rgba(20,68,71,0.8)' }}>
          ...
          <div className="grid md:grid-cols-2 gap-6 sm:gap-8 max-w-6xl mx-auto">
            ...
            <div>
              <h2 className="text-xl sm:text-2xl font-bold mb-3" style={{ color: C.darkTeal }}>Contact Information</h2>
              <p className="text-sm sm:text-base mb-6" style={{ color: 'rgba(20,68,71,0.7)' }}>
                ...
                {contactInfo.map((info, idx) => {
                  ...
                  return (
                    <motion.div key={idx} 
                      ...
                      style={{ background: G.glass80, backdropFilter: 'blur(20px)', border: `1px solid ${G.border60}` }}>
                      <div className="flex items-start gap-3 sm:gap-4">
                        ...
                        <div>
                          <h3 className="font-bold text-base sm:text-lg mb-1 sm:mb-2" style={{ color: C.darkTeal }}>{info.title}</h3>
                          {info.details.map((detail, i) => (
                            <p key={i} className="text-xs sm:text-sm" style={{ color: 'rgba(20,68,71,0.7)' }}>{detail}</p>
                          ))}
                        </div>
                      </div>
                    </motion.div>
              ...
              {/* Social Links */}
              <div className="mt-8">
                <h3 className="font-bold text-base sm:text-lg mb-4" style={{ color: C.darkTeal }}>Follow Us</h3>
                  ...
                  {socialLinks.map((social, idx) => {
                    ...
                    return (
                      <motion.a key={idx} href={social.link} target="_blank" rel="noopener noreferrer"
                        ...
                        style={{ background: G.glass80, backdropFilter: 'blur(20px)', color: C.teal, border: `1px solid ${G.border60}` }}>
                        <Icon size={16} />
            ...
            <motion.div 
              ...
              <div className="text-center mb-6">
                <h2 className="text-xl sm:text-2xl font-bold" style={{ color: C.darkTeal }}>Send us a Message</h2>
                <p className="text-xs sm:text-sm mt-2" style={{ color: 'rgba(20,68,71,0.7)' }}>We'd love to hear from you. Fill out the form and we'll respond shortly.</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs sm:text-sm font-semibold mb-2 flex items-center gap-2" style={{ color: C.darkTeal }}>
                    ...
                    <input type="text" name="name" value={formData.name} onChange={handleChange}
                      ...
                      style={{ background: 'rgba(255,255,255,0.5)', borderColor: 'rgba(43,135,141,0.4)', color: C.darkTeal }}
                ...
                <motion.button type="submit" disabled={isSubmitting} whileHover={{ y: -2 }}
                  ...
                  style={{ background: G.btn, color: 'white' }}>
                  {isSubmitting ? <>Sending... <Send size={18} /></> : <>Send Message <Send size={18} /></>}
                </motion.button>
              </form>
            </motion.div>
          </div>
          ...
          <section className="mt-16 sm:mt-20 max-w-6xl mx-auto">
            <div className="text-center mb-10 sm:mb-12">
              <h2 className="text-2xl sm:text-3xl font-bold mb-3 sm:mb-4" style={{ color: C.darkTeal }}>Frequently Asked Questions</h2>
              <p className="text-sm sm:text-base px-4" style={{ color: 'rgba(20,68,71,0.7)' }}>Find answers to common questions about our platform and services</p>
            </div>
            <div className="grid sm:grid-cols-2 gap-4 sm:gap-6">
              {faqs.map((faq, idx) => (
                <motion.div key={idx}
                  ...
                  style={{ background: G.glass80, backdropFilter: 'blur(20px)', border: `1px solid ${G.border60}` }}>
                  <h3 className="font-bold text-base sm:text-lg mb-2" style={{ color: C.darkTeal }}>{faq.question}</h3>
                  <p className="text-xs sm:text-sm leading-relaxed" style={{ color: 'rgba(20,68,71,0.7)' }}>{faq.answer}</p>
                </motion.div>
              ))}
            </div>
          </section>
          ...
          <motion.section 
            ...
            className="mt-16 sm:mt-20 text-center rounded-2xl p-8 sm:p-12 max-w-4xl mx-auto shadow-xl"
            style={{ background: G.darkSection }}>
            ...
            <motion.button whileHover={{ y: -2 }}
              ...
              Start 7-Day Challenge <Sparkles size={16} />
            </motion.button>
          </motion.section>
          ...
          <footer className="relative z-10 mt-16 sm:mt-20 py-8 sm:py-12 border-t" style={{ borderColor: 'rgba(255,255,255,0.4)' }}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center gap-4 sm:gap-6">
              <div className="flex items-center gap-2">
                <Globe className="w-5 h-5 sm:w-6 sm:h-6" style={{ color: C.teal }} />
