...
const GradientBg = () => (
  <div className="fixed inset-0 pointer-events-none z-0">
    <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, #EEF9FA 0%, #D4F0F3 50%, #E8F4F6 100%)' }} />
...
const getFallbackQuestions = (category, day) => {
  ...
  const base = {
    ...
    3: { title: "What You Can Be Paid For", type: "paid", questions: [
      ...
      { id: `fb_${category}_3_2`, text: "What value can you create that others would appreciate?", description: "Define your value proposition",
        options: [
    ...
    4: { title: "What The World Needs", type: "need", questions: [
      ...
      { id: `fb_${category}_4_2`, text: "What positive change do you want to create?", description: "Define your desired impact",
        options: [
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      >
        <motion.div
          ...
          className={`w-20 h-20 rounded-2xl bg-gradient-to-br ${c.color} flex items-center justify-center mx-auto mb-5 shadow-xl`}
        >
          {c.icon}
        </motion.div>
...
const CategorySelectionModal = ({ onSelectCategory, isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-lg">
      ...
      >
        {/* Header */}
        <div className="sticky top-0 bg-white/95 backdrop-blur-sm border-b border-gray-100 px-7 py-5 rounded-t-3xl z-10">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-[#64CDD1] to-[#5794A4] flex items-center justify-center shadow-lg shadow-cyan-200">
        ...
        <div className="p-7">
              ...
              return (
                <motion.button key={cat.id}
                  ...
                  className={`relative text-left p-6 rounded-2xl transition-all duration-250 border-2 ${
                    isSelected ? `bg-gradient-to-br ${cat.bg} ${cat.border} shadow-lg` : 'bg-gray-50 border-gray-100 hover:border-gray-200 hover:bg-white hover:shadow-md'
                  }`}
                >
                  {isSelected && (
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring' }}
                      className="absolute top-4 right-4 w-6 h-6 rounded-full flex items-center justify-center shadow-sm"
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {cat.tags.map(t => (
                      <span key={t} className="text-[10px] px-2 py-0.5 rounded-full bg-white text-gray-500 border border-gray-200 font-medium">{t}</span>
                    ))}
                  </div>
          ...
          <div className="mt-5 p-4 bg-gradient-to-r from-gray-50 to-slate-50 rounded-xl text-center border border-gray-100">
            <Quote size={14} className="text-gray-300 mx-auto mb-1.5" />
            <p className="text-xs text-gray-400 italic">"The only way to do great work is to love what you do."</p>
          </div>

          <div className="mt-5 flex gap-3">
            <button onClick={onClose}
              className="flex-1 py-3.5 border border-gray-200 text-gray-500 rounded-xl font-semibold text-sm hover:bg-gray-50 transition-all">
              Cancel
            </button>
            <motion.button
              onClick={() => { if (!selected || confirming) return; setConfirming(true); setTimeout(() => { onSelectCategory(selected); setConfirming(false); }, 400); }}
              disabled={!selected || confirming}
              className={`flex-[2] py-3.5 rounded-xl font-black text-sm flex items-center justify-center gap-2 transition-all ${
                selected && !confirming
            ...
            >
              {confirming ? <Loader2 size={16} className="animate-spin" /> : <><Rocket size={15} /> Begin Your Journey</>}
            </motion.button>
          </div>
        </div>
      </motion.div>
...
const DayHeader = ({ day, type, currentDayAnswers, totalQuestions }) => {
  ...
  const c = cfg[day] || cfg[1];
  const answered = Object.keys(currentDayAnswers || {}).length;
  const progress = totalQuestions > 0 ? (answered / totalQuestions) * 100 : 0;

  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', delay: 0.1 }}
        className="inline-flex items-center gap-3 bg-white/90 backdrop-blur-sm px-5 py-2.5 rounded-full mb-5 shadow-md border border-white/60">
        <div className={`w-8 h-8 rounded-xl bg-gradient-to-br ${c.grad} flex items-center justify-center text-white shadow-md`}>
          {c.icon}
        </div>
      ...
      <div className="mt-4 flex justify-center">
        <div className="relative w-14 h-14">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-[11px] font-black" style={{ color: c.color }}>{Math.round(progress)}%</span>
          </div>
        </div>
      </div>
    </motion.div>
...
const Sevendays = () => {
  ...
  useEffect(() => {
    if (selectedCategory && dayQuestions) {
      ...
      const first = dayQuestions.questions.findIndex(q => !contextAnswers[currentDay]?.[q.id]?.answer);
  ...
  const handleAnswerSelect = (answer) => {
    if (currentQuestion) {
      const updated = { ...currentAnswers, [currentQuestion.id]: { answer, type: currentType, timestamp: new Date().toISOString() } };
  ...
  const handleCompleteDay = useCallback(() => {
    if (isCompletingDay || isDayCompleted || !dayQuestions?.questions) return;
    const allAnswered = dayQuestions.questions.every(
      q => contextAnswers[currentDay]?.[q.id]?.answer || currentAnswers[q.id]?.answer
    );
    if (!allAnswered) { setToast('⚠️ Please answer all questions before completing the day.'); return; }

    setIsCompletingDay(true);
    completeDay(currentDay);  // ✅ Only marks complete — does NOT advance currentDay
  ...
  const handleNextQuestion = () => {
    if (!currentQuestionHasAnswer) { setToast('⚠️ Please select an answer first.'); return; }
    if (currentQuestionIndex < totalQuestions - 1) setCurrentQuestionIndex(p => p + 1);
    else handleCompleteDay();
  };
  ...
  // ✅ FIX: This is the ONLY place that advances currentDay.
  // completeDay() in context no longer does it, so there's no double-increment.
  ...
  const handleDownloadReport = () => {
    if (!result) return;
    const blob = new Blob([JSON.stringify({ selectedCategory, answers: contextAnswers, result, exportedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `ikigai-${new Date().toISOString().split('T')[0]}.json`; a.click();
  ...
  const handleResetJourney = () => {
    if (window.confirm('Reset your journey? All progress will be lost.')) {
      resetJourney(); setShowCategoryModal(true); setShowResult(false);
      setCurrentQuestionIndex(0); setCurrentAnswers({}); setCompletedDayNumber(null); setDayQuestions(null);
    }
  };
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 py-12">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', delay: 0.2 }}
            className="inline-flex items-center gap-2 bg-white/80 backdrop-blur-sm px-5 py-2.5 rounded-full mb-5 shadow-md border border-white/60">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
          ...
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
            ...
            className="px-8 py-3.5 bg-gradient-to-r from-[#5794A4] to-[#64CDD1] text-white rounded-xl font-bold shadow-lg shadow-cyan-200 hover:shadow-xl hover:shadow-cyan-300 transition-all">
            Start New Journey
          </motion.button>
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
            onClick={() => navigate('/')}
            className="px-8 py-3.5 border-2 border-[#5794A4] text-[#5794A4] rounded-xl font-bold hover:bg-[#5794A4] hover:text-white transition-all">
            Back to Home
          </motion.button>
        </motion.div>
      </div>
  ...
  if (!selectedCategory) return (
    <div className="min-h-screen relative flex items-center justify-center" style={{ marginLeft: 280 }}>
      <GradientBg />
      <CategorySelectionModal isOpen={showCategoryModal} onSelectCategory={handleCategorySelect} onClose={() => navigate('/')} />
    </div>
  );
  ...
  return (
    <div className="min-h-screen relative" style={{ marginLeft: 280 }}>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            className="fixed top-5 right-5 z-[100] px-4 py-3 rounded-xl text-sm font-semibold shadow-xl text-white flex items-center gap-2"
      ...
      <div className={`relative z-10 max-w-2xl mx-auto px-5 pt-8 pb-16 transition-all duration-600 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>

        {/* Reset */}
        <div className="flex justify-end mb-3">
          <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
            onClick={handleResetJourney}
            className="flex items-center gap-1.5 text-xs text-gray-300 hover:text-red-400 px-3 py-1.5 rounded-lg hover:bg-red-50 transition-all">
        ...
        <DayHeader
          day={currentDay} type={dayQuestions?.type}
        ...
        <motion.div
          ...
          className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 md:p-8 shadow-xl shadow-gray-100 border border-white/60"
        >
          {currentQuestion ? (
            <QuestionCard
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/60 backdrop-blur-sm rounded-full shadow-sm border border-white/50">
        ...
        {showConfetti && (
          <div className="fixed inset-0 pointer-events-none z-50">
            {Array.from({ length: 120 }).map((_, i) => (
              <motion.div key={i}
                initial={{ x: Math.random() * window.innerWidth, y: -10, rotate: 0, scale: Math.random() * 0.7 + 0.4 }}
                animate={{ y: window.innerHeight + 40, rotate: 360 * (Math.random() * 3 + 1), x: `+=${Math.random() * 80 - 40}` }}
                ...
                style={{
                  ...
                  background: ['#64CDD1', '#5794A4', '#FBBF24', '#F472B6', '#A78BFA', '#34D399', '#F87171'][Math.floor(Math.random() * 7)],
